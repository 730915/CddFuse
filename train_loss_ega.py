...
# 导入自定义的网络模型组件
from net_base_ega import Restormer_Encoder_EGA, Restormer_Decoder_EGA,BaseFeatureExtraction,DetailFeatureExtraction,EGAEnhancedBaseFeatureExtraction, EGAEnhancedDetailFeatureExtraction
...
# 初始化编码器并设置为并行模式
DIDF_Encoder = nn.DataParallel(Restormer_Encoder_EGA()).to(device)
# 初始化解码器并设置为并行模式（使用BiFPN_Decoder替换原来的Restormer_Decoder）
DIDF_Decoder = nn.DataParallel(Restormer_Decoder_EGA()).to(device)
# 初始化基础特征融合层
