...
from ImprovementModule.FCM import FCM
...
class Restormer_Encoder(nn.Module):
    def __init__(self,
        ...
        self.fcm = FCM(dim=dim)
                     
             
    def forward(self, inp_img, inp_img2=None):
        # 前向传播：提取基础特征和细节特征
        if inp_img2 is None:  # 单图像输入模式
            ...
            # 处理第一个输入（可见光）
            inp_enc_level1_vis = self.patch_embed(inp_img)  # 可见光图像块嵌入
            out_enc_level1_vis = self.encoder_level1(inp_enc_level1_vis)  # 可见光第一级编码
            
            # 处理第二个输入（红外）
            inp_enc_level1_ir = self.patch_embed(inp_img2)  # 红外图像块嵌入
            out_enc_level1_ir = self.encoder_level1(inp_enc_level1_ir)  # 红外第一级编码
            
            # 使用FCM模块融合两种特征
            out_enc_level1_vis, out_enc_level1_ir = self.fcm(out_enc_level1_vis, out_enc_level1_ir)
            
            # 从融合特征中提取基础特征和细节特征
            base_feature_vis = self.baseFeature(out_enc_level1_vis)  # 提取可见光融合特征的基础特征
            detail_feature_vis = self.detailFeature(out_enc_level1_vis)  # 提取可见光特征的细节特征
            
            base_feature_ir = self.baseFeature(out_enc_level1_ir)  # 提取红外光融合特征的基础特征
            detail_feature_ir = self.detailFeature(out_enc_level1_ir)  # 提取红外光融合特征的细节特征
            
            # 返回红外和可见光的特征
            return (base_feature_vis, detail_feature_vis, out_enc_level1_vis), \
                   (base_feature_ir, detail_feature_ir, out_enc_level1_ir)
...
def test_phase2_training():
    """
    测试第二阶段训练代码的正确性，特别是FCM模块的功能
    该函数创建随机输入并验证网络的第二阶段处理流程
    模拟train_loss.py中的第二阶段训练过程
    """
    import torch
    import torch.nn as nn
    ...
    # 创建随机输入
    data_VIS = torch.randn(batch_size, channels, height, width).cuda()
    data_IR = torch.randn(batch_size, channels, height, width).cuda()
    
    # 初始化编码器、解码器和融合层 - 与train_loss.py中使用的相同结构
    dim = 64
    DIDF_Encoder = Restormer_Encoder(inp_channels=channels, dim=dim).cuda()
    DIDF_Decoder = Restormer_Decoder(inp_channels=channels, out_channels=channels, dim=dim).cuda()
    
    # 初始化融合层 - 与train_loss.py中使用的相同结构
    BaseFuseLayer = BaseFeatureExtraction(dim=dim, num_heads=8).cuda()
    ...
    try:
        ...
        with torch.no_grad():
            ...
            print(f"   可见光基础特征形状: {feature_V_B.shape}, 可见光细节特征形状: {feature_V_D.shape}")
            print(f"   红外基础特征形状: {feature_I_B.shape}, 红外细节特征形状: {feature_I_D.shape}")
            
            # 融合基础特征 - 注意这里的特征已经通过FCM模块处理过
            print("2. 融合基础特征...")
            feature_F_B = BaseFuseLayer(feature_I_B+feature_V_B)
            print(f"   融合后的基础特征形状: {feature_F_B.shape}")
            
            # 融合细节特征 - 注意这里的特征已经通过FCM模块处理过
            print("3. 融合细节特征...")
            feature_F_D = DetailFuseLayer(feature_I_D+feature_V_D)
            print(f"   融合后的细节特征形状: {feature_F_D.shape}")
            
            # 使用融合特征生成融合图像
            print("4. 通过解码器生成融合图像...")
            data_Fuse, feature_F = DIDF_Decoder(data_VIS, feature_F_B, feature_F_D)
            print(f"   融合图像形状: {data_Fuse.shape}, 融合特征形状: {feature_F.shape}")
