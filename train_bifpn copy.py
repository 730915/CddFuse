...
# 导入BiFPN解码器替换原来的Restormer_Decoder
from net_with_bifpn import BiFPNDecoder,Restormer_Encoder, BaseFeatureExtraction, DetailFeatureExtraction
# 导入数据集处理类
...
DIDF_Decoder = nn.DataParallel(BiFPNDecoder()).to(device)
# 初始化基础特征融合层
...
if True:
    ...
    # 保存模型到文件，文件名包含时间戳
    torch.save(checkpoint, os.path.join("models/CDDFuse_BiFPN_"+timestamp+'.pth'))