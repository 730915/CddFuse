...
# 集成EGA模块的基础特征提取
class EGAEnhancedBaseFeatureExtraction(nn.Module):
    def __init__(self,
                 dim,
                 num_heads,
                 ffn_expansion_factor=1.,  
                 ...
                 feature_extra=True,
                 ega_position='after'):
        # EGA增强的基础特征提取模块：结合注意力机制、MLP和EGA，增强边缘特征提取能力
        # dim: 输入特征的通道数
        # num_heads: 注意力头的数量
        # ffn_expansion_factor: MLP中隐藏层的扩展倍数
        # qkv_bias: 是否在注意力机制中使用偏置
        # size, sigma, norm_layer, act_layer, feature_extra: EGA模块的参数
        # ega_position: EGA应用的位置，'before', 'after', 或 'both'
        super(EGAEnhancedBaseFeatureExtraction, self).__init__()
        self.norm1 = LayerNorm(dim, 'WithBias')  # 第一个层归一化
        self.attn = AttentionBase(dim, num_heads=num_heads, qkv_bias=qkv_bias,)  # 注意力模块
        self.norm2 = LayerNorm(dim, 'WithBias')  # 第二个层归一化
        self.mlp = Mlp(in_features=dim,
        ...
        self.ega = EdgeGaussianAggregation(dim=dim, size=size, sigma=sigma, 
                                         ...
                                         feature_extra=feature_extra)  # EGA模块
        self.ega_position = ega_position
                       
    def forward(self, x):
        # 前向传播实现残差连接的Transformer结构，并集成EGA模块
        if self.ega_position == 'before':
            # 先应用EGA增强，再进行特征提取
            x = self.ega(x)
            x = x + self.attn(self.norm1(x))
            x = x + self.mlp(self.norm2(x))
        elif self.ega_position == 'after':
            # 先进行特征提取，再应用EGA增强
            x = x + self.attn(self.norm1(x))
            x = x + self.mlp(self.norm2(x))
            x = self.ega(x)
...
# 集成EGA模块的细节节点
class EGAEnhancedDetailNode(nn.Module):
    def __init__(self, size=3, sigma=1.0, norm_layer=dict(type='BN', requires_grad=True), 
                 act_layer=nn.ReLU, feature_extra=True):
        # EGA增强的细节节点：在细节节点的基础上集成EGA模块，增强边缘特征提取
        super(EGAEnhancedDetailNode, self).__init__()
        # 三个变换函数，用于特征间的相互作用和信息流动
        self.theta_phi = InvertedResidualBlock(inp=32, oup=32, expand_ratio=2)
        self.theta_rho = InvertedResidualBlock(inp=32, oup=32, expand_ratio=2)
        self.theta_eta = InvertedResidualBlock(inp=32, oup=32, expand_ratio=2)
        ...
        self.shffleconv = nn.Conv2d(64, 64, kernel_size=1,
                                    stride=1, padding=0, bias=True)
        # EGA模块用于增强z1分支的边缘特征
        self.ega = EdgeGaussianAggregation(dim=32, size=size, sigma=sigma, 
                                         norm_layer=norm_layer, act_layer=act_layer, 
...
# 集成EGA模块的细节特征提取
class EGAEnhancedDetailFeatureExtraction(nn.Module):
    def __init__(self, num_layers=3, size=3, sigma=1.0, 
                 norm_layer=dict(type='BN', requires_grad=True), 
                 act_layer=nn.ReLU, feature_extra=True):
        # EGA增强的细节特征提取模块：堆叠多个EGAEnhancedDetailNode，增强边缘特征提取
        super(EGAEnhancedDetailFeatureExtraction, self).__init__()
        # 创建多个EGAEnhancedDetailNode实例并组成序列
        INNmodules = [EGAEnhancedDetailNode(size=size, sigma=sigma, 
        ...
        self.net = nn.Sequential(*INNmodules)
        # 最终的EGA模块用于整体增强
        self.final_ega = EdgeGaussianAggregation(dim=64, size=size, sigma=sigma, 
                                               norm_layer=norm_layer, act_layer=act_layer, 
                                               feature_extra=feature_extra)
        
    def forward(self, x):
        # 前向传播：将输入特征分成两半，依次通过每个EGAEnhancedDetailNode
        # 首先将特征分成两个分支
        z1, z2 = x[:, :x.shape[1]//2], x[:, x.shape[1]//2:x.shape[1]]
        # 依次通过每个EGAEnhancedDetailNode层
        for layer in self.net:
            z1, z2 = layer(z1, z2)
        # 连接两个分支的特征
        combined = torch.cat((z1, z2), dim=1)
        # 使用最终的EGA模块增强整体特征
        return self.final_ega(combined)
...
# 集成EGA模块的Transformer块
class EGAEnhancedTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type,
                 size=3, sigma=1.0, norm_layer=dict(type='BN', requires_grad=True),
                 act_layer=nn.ReLU, feature_extra=True, ega_position='after'):
        # EGA增强的Transformer块：结合自注意力、前馈网络和EGA，增强边缘特征提取能力
        super(EGAEnhancedTransformerBlock, self).__init__()
...
class Restormer_Encoder_EGA(nn.Module):
    def __init__(self,
                 inp_channels=1,
                 out_channels=1,
                 dim=64,
        ...
        # EGA增强的Restormer编码器：基于Transformer的图像编码器，集成EGA模块增强边缘特征
        super(Restormer_Encoder_EGA, self).__init__()

        # 图像块嵌入层：将输入图像转换为特征表示
        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)

        # 第一级编码器：堆叠多个EGA增强的Transformer块
        self.encoder_level1 = nn.Sequential(*[EGAEnhancedTransformerBlock(dim=dim, num_heads=heads[0], 
                                            ffn_expansion_factor=ffn_expansion_factor,
                                            bias=bias, LayerNorm_type=LayerNorm_type,
                                            size=size, sigma=sigma, norm_layer=norm_layer,
                                            act_layer=act_layer, feature_extra=feature_extra,
                                            ega_position=ega_position) 
                                            for i in range(num_blocks[0])])
        # 基础特征提取模块：捕获图像的全局结构和主要内容
        self.baseFeature = EGAEnhancedBaseFeatureExtraction(dim=dim, num_heads=heads[2],
        ...
        self.detailFeature = EGAEnhancedDetailFeatureExtraction(num_layers=3,
                                                              size=size, sigma=sigma,
                                                              norm_layer=norm_layer,
                                                              act_layer=act_layer,
                                                              feature_extra=feature_extra)
...
class Restormer_Decoder_EGA(nn.Module):
    def __init__(self,
                 inp_channels=1,
                 out_channels=1,
                 dim=64,
        ...
        # EGA增强的Restormer解码器：基于Transformer的图像解码器，集成EGA模块增强边缘特征
        super(Restormer_Decoder_EGA, self).__init__()
        # 通道减少层：将连接的特征通道数减半
        self.reduce_channel = nn.Conv2d(int(dim*2), int(dim), kernel_size=1, bias=bias)
        # 第二级编码器：堆叠多个EGA增强的Transformer块进行特征融合
        self.encoder_level2 = nn.Sequential(*[EGAEnhancedTransformerBlock(dim=dim, num_heads=heads[1], 
                                            ffn_expansion_factor=ffn_expansion_factor,
                                            bias=bias, LayerNorm_type=LayerNorm_type,
                                            size=size, sigma=sigma, norm_layer=norm_layer,
                                            act_layer=act_layer, feature_extra=feature_extra,
        ...
        # 最终的EGA模块：增强输出特征的边缘
        self.final_ega = EdgeGaussianAggregation(dim=dim, size=size, sigma=sigma, 
                                               norm_layer=norm_layer, act_layer=act_layer, 
                                               feature_extra=feature_extra)
        # 输出层：逐步减少通道数并生成最终输出
    ...
    def forward(self, inp_img, base_feature, detail_feature):
        ...
        # 应用最终的EGA模块增强边缘特征
        # out_enc_level1 = self.final_ega(out_enc_level1)
        # 生成输出，如果有输入图像则添加残差连接
...
if __name__ == '__main__':
    ...
    # 创建EGA增强的编码器和解码器模型实例并移至GPU
    modelE = Restormer_Encoder_EGA().cuda()  # 编码器
    modelD = Restormer_Decoder_EGA().cuda()  # 解码器
    ...
    print("EGA模块已成功集成到网络中，增强了边缘特征提取能力！")