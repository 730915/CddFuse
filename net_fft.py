...
class Restormer_Encoder(nn.Module):
    def __init__(self,
        ...
        self.freq_fusion = nn.Conv2d(dim*2, dim, kernel_size=1, bias=bias)
    ...
    def forward(self, inp_img):
        ...
        # 先对特征进行频率分解
        b, c, h, w = out_enc_level1.shape
        reshaped_feature = out_enc_level1.view(b, c, h*w)
        ...
        # 将低频部分传给BaseFeatureExtraction，高频部分传给DetailFeatureExtraction
        base_feature = self.baseFeature(low_freq)
        detail_feature = self.detailFeature(high_freq)
        
        # 融合处理后的特征
        enhanced_base = self.freq_fusion(torch.cat([base_feature, low_freq], dim=1))
        enhanced_detail = self.freq_fusion(torch.cat([detail_feature, high_freq], dim=1))
